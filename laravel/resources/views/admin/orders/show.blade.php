...
@section('title', 'Замовлення #' . $order->id . ' - Адмін-панель')

@section('content')
    <div class="max-w-4xl">
        <div class="mb-6">
            <a href="{{ route('admin.orders.index') }}" class="inline-flex items-center text-blue-600 hover:text-blue-800 transition">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 sm:p-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-6 border-b">
                ...
                <form action="{{ route('admin.orders.updateStatus', $order->id) }}" method="POST" class="w-full sm:w-auto">
                    @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                <div>
                    <h3 class="font-bold text-gray-900 mb-3">Інформація про клієнта</h3>
                    <div class="space-y-2">
                        <div>
                        ...
                        @if($order->user)
                            <div>
                                <p class="text-sm text-gray-600">ID користувача</p>
                                <p class="font-semibold">{{ $order->user->id }}</p>
                            </div>
                        @endif
                    </div>
                </div>
